...
date: '2026-05-06'
tags: ['MCP', 'Claude Code', 'Smart Home', 'TypeScript']
published: true
---

_Update, September 2026: this work grew into a real app. [Kettle Remote](https://kettle.michaelhutchinson.me) is now free on the [App Store](https://apps.apple.com/gb/app/kettle-remote/id6797603910) and [Google Play](https://play.google.com/store/apps/details?id=com.michaelhutchinson.kettleremote)._
...
I've built a couple of MCP servers before (one for [Strava](/blog/building-a-strava-mcp-server), one for [parkrun](/blog/parkrun-mcp-server)), so once the protocol was decoded and the TCP client worked, the MCP wrapper was the easy part. Five tools: `boil`, `stop`, `standby`, `status`, `discover`. Each maps to one Across command.
...
The MCP server is what I needed for Claude, but it's not the full picture. Two things were on the list:

- **My own phone app.** Done. I built it in React Native ([full write-up here](/blog/building-the-appkettle-app)).
- **Run it on a Pi.** The MCP server only works when I'm on my home network. Park it on a Raspberry Pi at home with a small remote endpoint and I can boil the kettle from anywhere. [Now underway as part of the smart home hub](/blog/building-a-raspberry-pi-smart-home-hub).
