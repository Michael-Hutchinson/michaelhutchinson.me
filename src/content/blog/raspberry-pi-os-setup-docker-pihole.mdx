...
**Clone the OS.** `rpi-clone`, run from the microSD system with the NVMe as the target (`sudo rpi-clone nvme0n1`). It copies the partition layout, clones the contents, and resizes the root partition to fill the drive.
...
Pi-hole's DNS engine defaults to only answering queries that look local, which a bridge-networked container never satisfies from the real LAN. Setting `DNSMASQ_LISTENING` to `all` in the compose environment fixes that. The firewall above does the actual access control; this just tells Pi-hole not to silently ignore queries it'd otherwise refuse.
