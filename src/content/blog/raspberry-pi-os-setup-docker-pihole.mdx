...
The reader arrived. Here's what it actually took to get from an empty SSD to a box I'd trust running 24/7 on my home network.
...
**Clone the OS.** SD Card Copier (`sudo apt install rpi-sd-copier`, then `picloner`). Source: microSD. Destination: NVMe. It copies the partition layout, clones the contents, and resizes the root partition to fill the drive.
...
I pinned Pi-hole to a specific version rather than `:latest`. Auto-updates can break things; I want to know when there's a new version and apply it deliberately. I also gave it a fixed IP on `hub-net`. Some services need to query Pi-hole directly by IP for DNS, and container names don't work for raw DNS lookups.

Pi-hole's DNS engine defaults to only answering queries that look local, which a bridge-networked container never satisfies from the real LAN. Setting `dns.listeningMode` to `ALL` in Pi-hole's config fixes that. The firewall above does the actual access control; this just tells Pi-hole not to silently ignore queries it'd otherwise refuse.
