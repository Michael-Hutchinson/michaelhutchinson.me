...
- **No secrets in tool responses.** MCP servers should sanitise responses. If a user record includes an API key hash, strip it before returning.
...
Build one MCP server, use it for a week, and you'll see why I [went all-in on this approach](/blog/ai-first-engineering). If you want a small, real example to start from, I've written up [a Strava server](/blog/building-a-strava-mcp-server) and [a parkrun server](/blog/parkrun-mcp-server), both published on npm. If you want to take it further, I wrote about [building production AI agents](/blog/ai-agents-that-ship-to-production) on top of this kind of infrastructure.
